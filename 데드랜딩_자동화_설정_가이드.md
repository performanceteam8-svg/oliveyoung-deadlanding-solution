...
- **표시 이름:** 데드랜딩 점검 결과 (성공/실패 동일)
- **본문 (성공):** ✅ 데드랜딩 점검에 성공했습니다. 결과를 확인해주세요.
- **본문 (실패):** ❌ 데드랜딩 점검에 실패했습니다. 원인을 확인해주세요.
- **버튼:** "결과 보기" → 스프레드시트 링크 (Variables에서 `SPREADSHEET_URL` 지정 가능, 없으면 기본 시트 URL 사용)

### 6.2 Slack 문구·버튼 URL을 변수로 지정

**Settings** → **Secrets and variables** → **Actions** → **Variables** 탭에서 추가 가능.

| 변수 이름 | 용도 |
|------------|------|
| `SPREADSHEET_URL` | "결과 보기" 버튼 링크 (예: `https://docs.google.com/spreadsheets/d/시트ID/edit`) |
| `SLACK_BODY_SUCCESS` | 성공 시 본문 (기본: 데드랜딩 점검에 성공했습니다. 결과를 확인해주세요.) |
| `SLACK_BODY_FAILURE` | 실패 시 본문 |
| `SLACK_FALLBACK_MSG` | 당일 6시 점검 결과 없을 때 문구 |
| `SLACK_USERNAME` | 메시지 표시 이름 (기본: 데드랜딩 점검 결과) |
